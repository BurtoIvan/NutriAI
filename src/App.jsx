...
const SIMPLE_MEALS = [
  "tostada","tostadas","café","mate","fruta","banana","manzana","naranja","yogur solo",
  "agua","té","infusión","gelatina","almendras","nueces","maní","naranja","mandarina"
];
...
function parseShopList(text) {
  ...
  section.split("\n").forEach(line => {
    ...
    if (!l.startsWith("-") && !l.startsWith("•") && l.length < 50) {
      ...
      if (name) items.push({ id: Math.random().toString(36).slice(2), cat, name, price: "" });
    }
  });
  return items;
}
...
function MealModal({ meal, day, profile, onClose, onSave }) {
  ...
  useEffect(() => {
    ...
    const sys = `Sos un nutricionista deportivo. Respondé en español argentino, de forma práctica y directa. Si es una receta, incluí ingredientes con cantidades, preparación paso a paso y al final: "MACROS: X kcal | Xg proteína | Xg carbohidratos | Xg grasas". Si el alimento es muy simple (tostada, fruta, café, mate), describilo brevemente sin pasos innecesarios.`;
    const usr = `Dame la receta o preparación de: "${meal}" (del plan del ${day}).
Perfil: ${profile.peso}kg, objetivo ${objMap[profile.objetivo]}, ${profile.calorias} kcal/día.`;
    callClaude(sys, usr)
      .then(r => { setText(r); setLoading(false); })
      .catch(() => { setText("❌ Error al cargar la receta. Intentá de nuevo."); setLoading(false); });
  }, []);
      ...
      <div className="modal-sheet">
        ...
        <div className="modal-inner">
          ...
          <div className="modal-sub">
            {day} · <span className={`tag-pill ${profile.objetivo}`}>
              {profile.objetivo === "vol" ? "Volumen" : profile.objetivo === "def" ? "Definición" : "Mantenimiento"}
            </span>
          </div>
          ...
          {loading ? (
            <div className="modal-loading">
              ...
              <div className="loading-text">Generando receta...</div>
            </div>
          ) : (
            <>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button className="btn-secondary" style={{ flex: 1 }} onClick={() => onSave(meal, text)}>
                  ⭐ Guardar
                </button>
...
export default function NutriAI() {
  ...
  const generateReceta = async () => {
    ...
    try {
      ...
      const sys = `Sos un nutricionista deportivo experto. Respondé en español argentino. Dá: nombre, ingredientes con cantidades, preparación paso a paso, y al final: "MACROS: X kcal | Xg proteína | Xg carbohidratos | Xg grasas". Sé práctico y motivador.`;
...
${ingredientes.length ? `Ingredientes en la heladera: ${ingredientes.join(", ")}` : ""}
${antojo ? `Antojo: ${antojo}` : ""}
Generá la receta ideal.`;
      const r = await callClaude(sys, usr);
      setHeladeraResult(r);
    } catch { setHeladeraResult("❌ Error al generar la receta. Intentá de nuevo."); }
    setHeladeraLoading(false);
  };

  const saveReceta = (nombre, texto) => {
    const n = nombre || texto.split("\n")[0].replace(/^#+\s*/, "").trim() || "Receta";
    setSaved(prev => [{ nombre: n, texto, date: new Date().toLocaleDateString("es-AR") }, ...prev]);
  };
  ...
  const generateSemana = async () => {
    ...
    try {
      const objMap = { vol: "volumen (superávit calórico, más proteína y carbos)", def: "definición (déficit moderado, alto proteína)", mant: "mantenimiento" };
      const sys = `Sos un nutricionista deportivo experto. Respondé en español argentino. Planificás semanas con alimentos comunes y accesibles en Argentina. Solo nombres concretos de platos, sin descripciones largas.`;
...
Generá un plan semanal (Lunes a Domingo) con exactamente ${profile.comidas} comidas/día.
Formato estricto:
LUNES
...
Al final, sección "LISTA DE COMPRAS" con ingredientes agrupados por categoría con cantidades para la semana.`;
      const raw = await callClaude(sys, usr);
      setSemanaData(parseSemana(raw));
      setShopList(parseShopList(raw));
      setChecked({});
    } catch { setSemanaData([]); }
  ...
  const totalEstimado = shopList.reduce((acc, i) => {
  ...
  const views = {
    heladera: (
      <div className="section">
        ...
        <div className="card">
          ...
          <textarea className="input-field" rows={2} placeholder="ej: algo con pasta, milanesas, ensalada..."
        ...
        {heladeraLoading && (
          <div className="loading-card">
            <div className="spinner" />
            <div className="loading-text">Pensando la receta perfecta...</div>
            <div className="loading-sub">Calculando macros según tu objetivo</div>
          </div>
        )}
            ...
            <div className="result-header">
              ...
              <div>
                ...
                <div className="result-meta">
                  <span className={`tag-pill ${profile.objetivo}`}>{OBJ_LABEL[profile.objetivo]}</span>
                </div>
              </div>
            </div>
    ...
    semana: (
      <div className="section">
        ...
        {semanaLoading && (
          <div className="loading-card">
            <div className="spinner" />
            <div className="loading-text">Armando tu plan semanal...</div>
            <div className="loading-sub">También se genera la lista de compras</div>
          </div>
        )}

        {semanaData && !semanaLoading && (
          semanaData.length === 0
            ? <div className="empty-state"><div className="empty-icon">😅</div><div className="empty-text">No se pudo parsear. Intentá de nuevo.</div></div>
            : (
              <>
                    ...
                    <div key={di} className="day-card">
                      ...
                      {d.meals.map((m, mi) => (
                        <div key={mi} className="meal-row">
                          ...
                          {!isSimpleMeal(m.name) && (
                            <button className="meal-tap-btn" onClick={() => setModal({ meal: m.name, day: d.day })}>
                              Ver receta
                            </button>
                          )}
                        </div>
                ...
                <button className="btn-secondary mt-8" style={{ width: "100%" }} onClick={generateSemana}>
                  🔄 Regenerar semana
                </button>
              </>
            )
      ...
      <div className="section">
        ...
        {shopList.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="empty-text">Generá tu plan semanal<br />y la lista aparece acá automáticamente</div>
          </div>
        ) : (
          <>
            <div className="total-card">
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>
                  {itemsConPrecio} de {shopList.length} con precio
                </div>
              </div>
              <div>
            ...
            <button className="btn-secondary" style={{ width: "100%", marginTop: 4 }} onClick={() => setChecked({})}>
              Resetear tachados
            </button>
          </>
        )}
    ...
    guardadas: (
      <div className="section">
        ...
        {saved.length === 0 ? (
          <div className="empty-state">
            <div className="empty-icon">⭐</div>
            <div className="empty-text">Guardá recetas desde Heladera<br />o desde el detalle de cualquier comida del plan</div>
          </div>
        ) : saved.map((r, i) => {
    ...
    perfil: (
      <div className="section">
        ...
        <div className="section-sub">Tus datos definen todas las recetas y planes</div>
          ...
          <div className="chips">
            {[{ id: "vol", label: "💪 Volumen" }, { id: "def", label: "🔥 Definición" }, { id: "mant", label: "⚖️ Mantenimiento" }].map(o => (
              <button key={o.id} className={`chip ${o.id} ${profile.objetivo === o.id ? "selected" : ""}`}
                onClick={() => setProfile({ ...profile, objetivo: o.id })}>{o.label}</button>
            ))}
          </div>
        </div>
          ...
          <div className="chips">
            {[3, 4, 5, 6].map(n => (
              <button key={n} className={`chip ${profile.comidas === n ? "selected" : ""}`}
                onClick={() => setProfile({ ...profile, comidas: n })}>{n} comidas</button>
            ))}
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="slider-row">
            <input type="range" min={1200} max={4000} step={50} value={profile.calorias}
              onChange={e => setProfile({ ...profile, calorias: +e.target.value })} />
        ...
        <div className="card">
          ...
          <input className="input-field" placeholder="ej: sin gluten, vegetariano, sin lactosa..."
            value={profile.restricciones} onChange={e => setProfile({ ...profile, restricciones: e.target.value })} />
        </div>
      </div>
    ),
  };
